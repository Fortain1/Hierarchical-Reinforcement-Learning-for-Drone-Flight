...
class ObjectHandler:
    ...
    def __init__(
        ...
        self.obs_obj_dir = os.path.join(file_dir, "assets\\obstacle.urdf")
    ...
    def reset(
        ...
        if self.enable_render:
            ...
            for obstacle in self.obstacles:
                self.obstacle_visual.append(
                    self.p.loadURDF(
                        self.obs_obj_dir,
                        basePosition=obstacle,
                        useFixedBase=True,
                        globalScaling= 1,
                    )
                )
            for id in self.obstacle_visual:
                self.p.changeVisualShape(
                    id,
                    linkIndex=-1,
                    rgbaColor=(1, 0, 0, 1),
    ...
    def distance_to_obstacle(self, lin_pos, quarternion):
        rotation = np.array(self.p.getMatrixFromQuaternion(quarternion)).reshape(3, 3)
        obstacle_deltas = np.matmul((self.obstacles - lin_pos), rotation)
        
        distances = np.linalg.norm(obstacle_deltas,axis=1)

        index_min = np.argmin(distances)

        return obstacle_deltas[index_min]
